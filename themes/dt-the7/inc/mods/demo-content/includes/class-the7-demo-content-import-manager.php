...
If the problem persists, please don\'t hesitate to contact our <a href="%s" target="_blank">support</a>.', 'the7mk2' ), 'http://support.dream-theme.com/' ) );
	...
	public function import_elementor_settings() {
		...
		$elementor_site_meta = $site_meta['elementor'];
		...
		foreach ( $elementor_settings_to_import as $elementor_setting ) {
			if ( isset( $elementor_site_meta[ $elementor_setting ] ) ) {
				update_option( $elementor_setting, $elementor_site_meta[ $elementor_setting ] );
			} else {
		...
		if (
			isset( $site_meta['elementor_kit_settings'] )
			...
			$current_settings = $kit->get_meta( \Elementor\Core\Settings\Page\Manager::META_KEY );

			$elementor_kit_settings = $site_meta['elementor_kit_settings'];
			foreach ( $kit_settings as $kit_setting ) {
				if ( isset( $elementor_kit_settings[ $kit_setting ] ) ) {
					$current_settings[ $kit_setting ] = $elementor_kit_settings[ $kit_setting ];
				} else {
					unset( $current_settings[ $kit_setting ] );
				}
			}
